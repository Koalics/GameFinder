...
import {
  getTopListConfig,
  resolveListSlugFromRoute,
} from '../data/topLists.js';
...
async function loadGames(append) {
  ...
  try {
    ...
    const totalAfter = append
      ? games.value.length + batch.length
      : batch.length;
    hasMore.value =
      Boolean(next) && batch.length > 0 && (!maxGames || totalAfter < maxGames);
    page.value = nextPage;
